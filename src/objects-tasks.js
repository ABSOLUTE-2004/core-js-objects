...
function compareObjects(obj1, obj2) {
  ...
  const unmatchedKeys = keys1.filter((key) => obj1[key] !== obj2[key]);
...
function sellTickets(queue) {
  ...
  const isPossible = queue.every((bill) => {
    counterOfBills[bill] += 1;

    if (bill === 50) {
      if (counterOfBills[25] > 0) {
        counterOfBills[25] -= 1;
        return true;
      }
      return false;
    ...
    if (bill === 100) {
      if (counterOfBills[50] > 0 && counterOfBills[25] > 0) {
        counterOfBills[50] -= 1;
        counterOfBills[25] -= 1;
        return true;
      }
      if (counterOfBills[25] >= 3) {
        counterOfBills[25] -= 3;
        return true;
      }
      return false;
